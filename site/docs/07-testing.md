...
After it, the database is empty — no entities, no data, no tables. Follow it with a
`push` to recreate the schema, then seed through your normal routes.
...
- **Set** (truthy — `true`, `1`, `yes`, `on`) → reset is allowed.
- **Unset** → the endpoint responds `403` and **does nothing**.
...
```ts
import { createClient, pushEntities } from "@mauroandre/weave-sdk";
...
export async function globalSetup() {
  ...
  await pushEntities(entities, cfg);  // → rebuild schema from code
