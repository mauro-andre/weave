...
import { array, defineEntity, owned, text, weave, type Weave } from "../../src/index.js";
...
describe.skipIf(noDb)("find (weave read)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.sql`drop table if exists
      weave_find_user_addresses_landmarks,
      weave_find_user_addresses,
    ...
    await db.transaction(async (tx) => {
      ...
      const [a] = await tx<{ id: string }[]>`
        insert into weave_find_user_addresses (user_id, street)
        values (${u!.id}, 'Rua X') returning id`;
      await tx`insert into weave_find_user_addresses_landmarks (address_id, label)
        values (${a!.id}, 'Esquina')`;
    });
  });
