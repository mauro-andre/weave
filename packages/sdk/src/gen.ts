...
/** Client configurado (`weave/index.ts`) — lê WEAVE_URL/WEAVE_KEY do ambiente. Com
 *  `ambient`, gera o client multi-tenant (`createAmbientClient` do subpath `/als`). */
function clientSource(ambient = false): string {
  const factory = ambient ? "createAmbientClient" : "createClient";
  const importLine = ambient
    ? `import { createAmbientClient } from "@mauroandre/weave-sdk/als";`
...
export interface GenOptions {
  ...
  key: string;
  fetch?: FetchLike;
  /** Gera o `weave` como client ambient (multi-tenant). Vem do `weave.config { ambient }`. */
  ambient?: boolean;
}
...
export async function genProject(options: GenOptions): Promise<GenProject> {
  ...
  files["index.ts"] = clientSource(options.ambient);
